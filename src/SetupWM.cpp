...
//-------------------------------------------------------
void SearchWM(uint64_t Rx1, uint64_t Tx, uint8_t lCh, uint8_t ladr) {  
...
void SetWM(uint64_t Rx1, uint64_t Tx, uint8_t lCh, uint8_t ladr, uint64_t sRx1, uint64_t sTx1,
 ...
 setupWM.RxTube1 = 0; 
 setupWM.slpDelay = 0;
 setupWM.StrtAdrSlv = 0;
 setupWM.NSlv = 0;
 setupWM.SlTimeout = 0;
 setupWM.TOpenClose = 0;
 ...
 SLpkt.SrcAdr = Address;                 // кто      
 SLpkt.DstAdr = ladr;                    // кому        
 SLpkt.CMD = 4;                          // Зададим CMD
 (uint32_t&)SLpkt.buf[0] = topen; 
 SendPacket(1); 
 interrupts();
 delay(200); 
 noInterrupts();
 SLpkt.SrcAdr = Address;                 // кто      
 SLpkt.DstAdr = ladr;                    // кому        
 SLpkt.CMD = 5;                          // Зададим CMD
 (uint8_t&)SLpkt.buf[0] = ma;
 (int16_t&)SLpkt.buf[1] = nslv;  
 (uint32_t&)SLpkt.buf[2] = toutslv;
 (uint8_t&)SLpkt.buf[3] = aslv1;
 SendPacket(1); 
 interrupts();
 delay(200); 
 noInterrupts();
 SLpkt.SrcAdr = Address;                 // кто      
 SLpkt.DstAdr = ladr;                    // кому        
 SLpkt.CMD = 6;                          // Зададим CMD
 (uint64_t&)SLpkt.buf[0] = sTx2; 
 SendPacket(1); 
 interrupts();
 delay(200);
 noInterrupts();
 SLpkt.SrcAdr = Address;                 // кто      
 SLpkt.DstAdr = ladr;                    // кому        
 SLpkt.CMD = 7;                          // Зададим CMD
 (uint64_t&)SLpkt.buf[0] = sRx2; 
 SendPacket(1); 
 interrupts();
 delay(200);
 TxTube = oldTxTube; 
...
//-------------------------------------------------------
void StoreWM(uint64_t Rx1, uint64_t Tx, uint8_t lCh, uint8_t ladr) {
 uint64_t oldTxTube, oldRxTube1;
 uint8_t oldChannel;
